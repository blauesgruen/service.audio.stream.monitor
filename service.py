...
from constants import (
    ...
    API_NOW_REFRESH_INTERVAL_S, QF_NO_RESPONSE_FALLBACK_S, QF_ERROR_FALLBACK_S, QF_HIT_MISMATCH_GRACE_S,
    ...
    SOURCE_GROUP_DB_SWAP_MIN_SAMPLES, SOURCE_GROUP_DB_SWAP_MIN_SHARE,
...
class RadioMonitor(xbmc.Monitor):
    ...
    QF_ERROR_FALLBACK_S = float(QF_ERROR_FALLBACK_S)
    ...
    def __init__(self):
        ...
        self._qf_error_streak_since_ts = 0.0
    ...
    def _record_source_group_decision_if_allowed(self, station_name, decision_source):
        ...
        try:
            ...
            if ok:
                ...
                self._update_source_swap_status_property(
    ...
    def _persist_qf_degraded_state_if_allowed(self, station_name='', degraded=False, reason=''):
        if not (self._persist_data and self._profile_store):
            return False
        station_key = self._build_station_profile_key(station_name)
        if not station_key:
            station_key = str(self._active_station_profile_key or '')
        if not station_key:
            return False
        try:
            return bool(
                self._profile_store.set_qf_degraded_state(
                    station_key,
            ...
            log_debug(f"QF-Degrade Persist fehlgeschlagen: {e}")
            return False

    def _apply_persisted_qf_degraded_hint(self, station_name='', station_key=''):
        if self._profile_store is None:
            return False
        key = str(station_key or self._build_station_profile_key(station_name) or '').strip()
        if not key:
            return False
        try:
            state = self._profile_store.get_qf_degraded_state(key) or {}
        except Exception as e:
            log_debug(f"QF-Degrade Read fehlgeschlagen: {e}")
            return False
        if not bool(state.get('qf_degraded')):
            return False
        ...
        self._qf_last_probe_ts = 0.0
        if station_name:
            self._last_qf_request_station = str(station_name or '')
        degraded_at_ts = int(state.get('degraded_at_ts', 0) or 0)
        persisted_age_s = max(0, int(time.time()) - degraded_at_ts) if degraded_at_ts > 0 else 0
        self._log_qf_diag('degrade_hint_applied', {
            'station_key': key,
            'reason': str(state.get('degrade_reason') or 'persisted_qf_degraded'),
            'persisted_age_s': persisted_age_s,
            'probe_interval_s': self.QF_DEGRADE_PROBE_INTERVAL_S,
        }, level='info')
        return True

    def _update_source_swap_status_property(self, station_name='', decision_source=''):
        """
        Spiegelt den DB-basierten Swap-Status als Label.
        Format: '<family>:<swapped_wins>/<wins> (<share>)'
        Beispiel: 'icy:12/40 (0.300)'
        """
        if self._profile_store is None:
            WINDOW.clearProperty(_P.SOURCE_SWAP_STATUS)
            return

        station_name_text = str(station_name or WINDOW.getProperty(_P.STATION) or '').strip()
        station_key = self._build_station_profile_key(station_name_text)
        if not station_key:
            WINDOW.clearProperty(_P.SOURCE_SWAP_STATUS)
            return

        try:
            stats = self._profile_store.get_source_family_stats(station_key)
        except Exception:
            stats = {}
        ...
        source_text = str(decision_source or WINDOW.getProperty(_P.SOURCE) or '').strip()
        family = self._source_family(source_text)
        row = stats.get(family) if family else None
        if not isinstance(row, dict):
            row = None
            best_family = ''
            best_wins = -1
            for fam, fam_row in stats.items():
                if not isinstance(fam_row, dict):
                    continue
                wins = int(fam_row.get('wins', 0) or 0)
                if wins > best_wins:
                    best_wins = wins
                    best_family = str(fam or '')
                    row = fam_row
            family = best_family
        ...
        self.set_property_safe(_P.SOURCE_SWAP_STATUS, f"{family}:{swapped_wins}/{wins} ({share:.3f})")
    ...
    def _prefer_source_swapped_from_db(self, source_family, station_name=''):
        ...
        if not station_key:
            station_key = self._build_station_profile_key(station_name or WINDOW.getProperty(_P.STATION) or '')
        if not station_key:
            return False
        try:
            stats = self._profile_store.get_source_family_stats(station_key)
    ...
    def _refresh_station_profile_context(self, station_name='', enable_policy=False):
        ...
        if key_changed:
            ...
            self._update_source_swap_status_property(station_name=station_name)
            self._apply_persisted_qf_degraded_hint(station_name=station_name, station_key=key)
    ...
    def _qf_start_error_streak(self, base_ts=0.0):
        if self._qf_error_streak_since_ts > 0.0:
            return
        start_ts = float(base_ts or time.time())
        self._qf_error_streak_since_ts = max(0.0, start_ts)
        self._log_qf_diag('error_streak_start', {
            'since_ts': int(self._qf_error_streak_since_ts),
            'fallback_s': self.QF_ERROR_FALLBACK_S,
            'req_id': self._last_qf_request_id or '-',
            'res_id': self._last_qf_response_id or '-',
        })

    def _qf_clear_error_streak(self, reason=''):
        if self._qf_error_streak_since_ts <= 0.0:
            return
        age_s = max(0.0, float(time.time() - float(self._qf_error_streak_since_ts)))
        self._qf_error_streak_since_ts = 0.0
        self._log_qf_diag('error_streak_end', {
            'age_s': age_s,
            'reason': reason or '-',
            'req_id': self._last_qf_request_id or '-',
            'res_id': self._last_qf_response_id or '-',
    ...
    def _qf_error_streak_age_s(self):
        if self._qf_error_streak_since_ts <= 0.0:
            return 0.0
        return max(0.0, float(time.time() - float(self._qf_error_streak_since_ts)))

    @staticmethod
    def _is_qf_fresh_external_error(response_status, response_source):
        return (
            str(response_source or '').strip().lower() != 'asm'
            and str(response_status or '').strip().lower() == 'error'
        )

    def _qf_enter_degraded_mode(self, reason='', degrade_after_s=None):
        if self._qf_degraded_mode_active:
            return
        self._qf_degraded_mode_active = True
        self._qf_degraded_since_ts = time.time()
        self._qf_last_probe_ts = 0.0
        self._persist_qf_degraded_state_if_allowed(
            station_name=WINDOW.getProperty(_P.STATION) or '',
            degraded=True,
            reason=reason or 'no_response_over_threshold',
        )
        threshold_s = (
            float(degrade_after_s)
            if isinstance(degrade_after_s, (int, float)) and float(degrade_after_s) > 0.0
            else float(self.QF_NO_RESPONSE_DEGRADE_S)
        )
        self._log_qf_diag('degrade_start', {
            'reason': reason or 'no_response_over_threshold',
            'degrade_s': threshold_s,
            'probe_interval_s': self.QF_DEGRADE_PROBE_INTERVAL_S,
            'req_id': self._last_qf_request_id or '-',
            'res_id': self._last_qf_response_id or '-',
        }, level='info')
    ...
    def _qf_exit_degraded_mode(self, reason=''):
        ...
        self._qf_clear_error_streak(reason=reason or 'degrade_exit')
    ...
    def _should_force_qf_apply_on_pair_change(self, last_applied_qf_pair, fresh_qf_pair):
        if not self._qf_enabled:
            return False
        if not (fresh_qf_pair and fresh_qf_pair[0] and fresh_qf_pair[1]):
            return False
        prev_artist = str((last_applied_qf_pair or ('', ''))[0] or '').strip()
        prev_title = str((last_applied_qf_pair or ('', ''))[1] or '').strip()
    ...
    def _send_qf_request(self, station_name, mode='asm_auto', station_id=''):
        ...
        log_debug(
            ...
            f"station_id='{station_id_text or '-'}', mode='{mode}'"
        )
        return True
    ...
    def _sync_qf_result_property(self):
        ...
        if not self._qf_enabled or not self.is_playing:
            ...
            self._qf_clear_error_streak(reason='qf_disabled_or_not_playing')
        ...
        if not self._last_qf_request_id:
            ...
            self._qf_clear_error_streak(reason='no_request_id')
            WINDOW.clearProperty(_P.QF_RESULT)
            return
        ...
        if self._is_qf_fresh_external_error(response_status, response_source):
            self._qf_clear_no_response_streak(reason='fresh_qf_error_response')
            self._qf_start_error_streak(base_ts=time.time())
            error_age_s = self._qf_error_streak_age_s()
            if error_age_s >= float(self.QF_ERROR_FALLBACK_S):
                self._qf_enter_degraded_mode(
                    reason='error_over_threshold',
                    degrade_after_s=self.QF_ERROR_FALLBACK_S,
                )
        else:
            self._qf_clear_error_streak(reason='fresh_non_error_qf_response')
            if response_source != 'asm':
                self._persist_qf_degraded_state_if_allowed(
                    station_name=WINDOW.getProperty(_P.STATION) or '',
                    degraded=False,
                    reason='fresh_qf_response',
                )
                if self._qf_degraded_mode_active:
        ...
        self._last_qf_fresh_hit_pair = (artist, title)
    ...
    def parse_stream_title(self, stream_title, station_name=None, stream_url=None):
        ...
        if stream_candidates and evaluations and (mb_all_zero or mb_no_winner_with_swap_hint):
            ...
            if locked_source_family and locked_source_pair[0] and locked_source_pair[1]:
                if (
                    ...
                    log_info(
                        "MB ohne Winner, Source-Lock='icy' liefert generische/Nicht-Song-Daten -> "
                        "kein Song, nutze nur Station/StreamTitle"
                    )
                    self._set_last_song_decision('', None, None)
                    return None, None, '', '', '', '', 0
                log_info(
                    f"MB ohne Winner, Source-Lock='{locked_source_family}' "
            ...
            if has_icy_candidate:
                ...
                if icy_fallback_ev:
                    ...
                    _t = icy_fallback_ev.get('input_title')
                    _icy_source = str(icy_fallback_ev.get('source') or 'icy')
                else:
                    _a, _t, _icy_source = None, None, 'icy'
                if _a and _t:
                    if (
                        ...
                        log_info(
                            "MB ohne Winner, ICY-Rohdaten sind generisch/Nicht-Song -> "
                            "kein Song, nutze nur Station/StreamTitle"
                        )
                        self._set_last_song_decision('', None, None)
                        return None, None, '', '', '', '', 0
                    ...
                    if _pure_num.match(_a) or _pure_num.match(_t):
                        ...
                        log_info(
                            f"MB ohne Winner, kein API – ICY-Rohdaten-Fallback: "
                            f"'{_a} - {_t}' (source={_icy_source})"
                        )
                        self._set_last_song_decision(_icy_source, _a, _t)
                        if self._is_mp_decision_active():
                            self._log_musicplayer_comparison(_icy_source, (_a, _t), mp_pairs)
                            self._update_musicplayer_trust_after_decision(_icy_source, (_a, _t), mp_pairs)
                        return _a, _t, '', '', '', '', 0
            log_info(
                "MB ohne Winner, keine belastbaren Songdaten -> "
                ...
                try:
                    ...
                    if (
                        ...
                        if self._qf_enabled:
                            ...
                            qf_context = self._current_qf_hit_context(
                                require_fresh=qf_require_fresh,
                            ...
                            if isinstance(fresh_qf_context, dict):
                                fresh_qf_pair = (
                                    str(fresh_qf_context.get('artist') or '').strip(),
                                    str(fresh_qf_context.get('title') or '').strip(),
                                )
                            if not (fresh_qf_pair[0] and fresh_qf_pair[1]):
                                # Fresh-Hits koennen durch Poll/Loop-Rennen kurzzeitig nicht sichtbar sein.
                                # Das letzte frische QF-Paar wird deshalb als Trigger-Anker genutzt.
                                fresh_qf_pair = self._last_qf_fresh_hit_pair
                            qf_authoritative = self._is_qf_authoritative()
                        ...
                        if (
                            ...
                            self._log_qf_diag('force_apply_fresh_hit_changed_pair', {
                                ...
                                'prev_pair': self._label_from_pair(self._last_qf_applied_pair) or '-',
                        ...
                        if source_changed_trigger:
                            ...
                            if str(decision_source or '').startswith('asm-qf') and not (artist and title):
                                # QF-Paare nur atomar fuehren: nie halbe Labels stehen lassen.
                                WINDOW.clearProperty(_P.TITLE)
                                WINDOW.clearProperty(_P.ARTIST)
                                WINDOW.clearProperty(_P.ARTIST_DISPLAY)
                                title = ''
                            ...
                            if decision_source:
                                ...
                                if str(decision_source).startswith('asm-qf'):
                                    # Fuer Force-Apply das rohe QF-Entscheidungspaar verwenden,
                                    # nicht das ggf. MB-korrigierte Ausgabepaar.
                                    if decision_pair[0] and decision_pair[1]:
                                        self._last_qf_applied_pair = (
                                            decision_pair[0] or '',
                                            decision_pair[1] or '',
